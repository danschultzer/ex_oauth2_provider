defmodule ExOauth2Provider.Token.Utils.Response do
  ...
  def response({:ok, %{access_token: token, id_token: id_token}}, config),
    do: build_response(%{access_token: token, id_token: id_token}, config)
  ...
  defp build_response(%{access_token: access_token} = token, config) do
    body =
      %{
  ...
  defp set_id_token(body, %{id_token: id_token}), do: Map.put(body, :id_token, id_token)
  defp set_id_token(body, _), do: body
