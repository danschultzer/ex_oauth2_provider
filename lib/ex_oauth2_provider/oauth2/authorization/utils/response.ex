defmodule ExOauth2Provider.Authorization.Utils.Response do
  ...
  def error_response({:error, %{error: error} = params}, config),
  ...
  def preauthorize_response({:ok, %{client: client, request: %{"scope" => scopes}}}, _config),
    do: {:ok, client, Scopes.to_list(scopes)}

  def preauthorize_response({:error, %{error: error} = params}, config),
    do: build_response(params, error, config)
  ...
  def deny_response({:error, %{error: error} = params}, config),
    do: build_response(params, error, config)
