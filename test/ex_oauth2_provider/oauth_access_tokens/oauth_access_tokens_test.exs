defmodule ExOauth2Provider.OauthAccessTokensTest do
  ...
  describe "create_token/2" do
    ...
    test "with application", %{application: application} do
  ...
  describe "get_or_create_token/2" do
    ...
    test "creates token when params are different", %{user: user} do
      ...
      {:ok, access_token3} = OauthAccessTokens.get_or_create_token(user, another_application, nil, %{})
      assert access_token3.id != access_token.id

      {:ok, access_token4} = OauthAccessTokens.get_or_create_token(user, nil, "read", %{})
