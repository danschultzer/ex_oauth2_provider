defmodule ExOauth2Provider.AccessTokensTest do
  ...
  test "get_by_token/2", %{user: user} do
    assert {:ok, access_token} =
             AccessTokens.create_token(user, %{}, otp_app: :ex_oauth2_provider)

    assert %OauthAccessToken{id: id} =
             AccessTokens.get_by_token(access_token.token, otp_app: :ex_oauth2_provider)
  ...
  describe "get_by_previous_refresh_token_for/3" do
    test "with resource owner", %{user: user} do
      {:ok, old_access_token} =
        AccessTokens.create_token(user, %{use_refresh_token: true}, otp_app: :ex_oauth2_provider)

      {:ok, new_access_token} =
        AccessTokens.create_token(
          user,
          %{use_refresh_token: true, previous_refresh_token: old_access_token},
          otp_app: :ex_oauth2_provider
        )

      assert %OauthAccessToken{id: id} =
               AccessTokens.get_by_previous_refresh_token_for(new_access_token,
                 otp_app: :ex_oauth2_provider
               )
      ...
      {:ok, new_access_token_different_user} =
        AccessTokens.create_token(
          Fixtures.resource_owner(),
          %{use_refresh_token: true, previous_refresh_token: old_access_token},
          otp_app: :ex_oauth2_provider
        )
    ...
    test "with application", %{user: user, application: application} do
      {:ok, old_access_token} =
        AccessTokens.create_token(user, %{application: application, use_refresh_token: true},
          otp_app: :ex_oauth2_provider
        )
      ...
      {:ok, new_access_token} =
        AccessTokens.create_token(
          ...
          %{
            ...
            previous_refresh_token: old_access_token
          },
          otp_app: :ex_oauth2_provider
        )
      ...
      {:ok, new_access_token_different_app} =
        AccessTokens.create_token(
          user,
          %{
  ...
  describe "get_token_for/4" do
    test "fetches for resource owner", %{user: user, application: application} do
      ...
      {:ok, _access_token_with_application} =
        AccessTokens.create_token(user, %{application: application}, otp_app: :ex_oauth2_provider)

      assert %OauthAccessToken{id: id} =
               AccessTokens.get_token_for(user, nil, nil, otp_app: :ex_oauth2_provider)
    ...
    test "with application", %{user: user, application: application} do
      ...
      {:ok, _access_token_without_application} =
        AccessTokens.create_token(user, %{}, otp_app: :ex_oauth2_provider)

      assert %OauthAccessToken{id: id} =
               AccessTokens.get_token_for(user, application, nil, otp_app: :ex_oauth2_provider)
    ...
    test "with scopes", %{user: user} do
      {:ok, access_token1} =
        AccessTokens.create_token(user, %{scopes: "public"}, otp_app: :ex_oauth2_provider)

      {:ok, access_token2} =
        AccessTokens.create_token(user, %{scopes: "read write"}, otp_app: :ex_oauth2_provider)

      assert %OauthAccessToken{id: id} =
               AccessTokens.get_token_for(user, nil, "public", otp_app: :ex_oauth2_provider)

      assert id == access_token1.id

      assert %OauthAccessToken{id: id} =
               AccessTokens.get_token_for(user, nil, "write read", otp_app: :ex_oauth2_provider)
    ...
    test "filters expired", %{user: user} do
      ...
      inserted_at =
        QueryHelpers.timestamp(access_token.__struct__, :inserted_at,
  ...
  describe "get_application_token_for/3" do
    test "fetches", %{application: application} do
      ...
      refute AccessTokens.get_application_token_for(
               Fixtures.application(uid: "application-2"),
               nil,
               otp_app: :ex_oauth2_provider
             )
    end
  ...
  test "get_authorized_tokens_for/2", %{user: user, application: application} do
    ...
    assert [%OauthAccessToken{id: id}] =
             AccessTokens.get_authorized_tokens_for(user, otp_app: :ex_oauth2_provider)
  ...
  describe "create_token/3" do
    ...
    test "with custom access token generator", %{user: user} do
      {:ok, access_token} =
        AccessTokens.create_token(user, %{},
          otp_app: :ex_oauth2_provider,
          access_token_generator: {__MODULE__, :access_token_generator}
        )
    ...
    test "doesn't add refresh token when disabled", %{user: user} do
      {:ok, access_token} =
        AccessTokens.create_token(user, %{use_refresh_token: false}, otp_app: :ex_oauth2_provider)
    ...
    test "with custom scopes", %{user: user} do
      assert {:ok, access_token} =
               AccessTokens.create_token(user, %{scopes: "read"}, otp_app: :ex_oauth2_provider)
    ...
    test "with invalid scopes", %{user: user} do
      assert {:error, changeset} =
               AccessTokens.create_token(user, %{scopes: "invalid"}, otp_app: :ex_oauth2_provider)
  ...
  describe "create_token/3 with application scopes" do
    ...
    test "with custom scopes", %{user: user, application: application} do
      ...
      assert {:ok, access_token} =
               AccessTokens.create_token(user, %{scopes: "app:read", application: application},
                 otp_app: :ex_oauth2_provider
               )
    ...
    test "with invalid scopes", %{user: user, application: application} do
      ...
      assert {:error, changeset} =
               AccessTokens.create_token(user, %{application: application, scopes: "app:write"},
                 otp_app: :ex_oauth2_provider
               )
  ...
  test "is_revoked?/1" do
    assert AccessTokens.is_revoked?(%OauthAccessToken{
             revoked_at: QueryHelpers.timestamp(OauthAccessToken, :revoked_at)
           })
  ...
  describe "is_accessible?/1" do
    test "with active" do
      access_token = %OauthAccessToken{
        expires_in: 1,
        revoked_at: nil,
        inserted_at: QueryHelpers.timestamp(OauthAccessToken, :inserted_at)
      }
